function Get-CurrentLoad {
    ...
    .PARAMETER path
        ...
        Vychozi je hostname_datummereni.csv v aktualnim pracovnim adresari.
    ...
    .EXAMPLE
        ...
        Vypise aktualni zatizeni a navic vysledky ulozi do hostname_datummereni.csv.
    ...
    param (
        ...
        ,
        [string] $path = ".\$computerName`_$(Get-Date -f MMddyyyyHHmms).csv"
    )
    ...
    process {
        Invoke-Command2 -computerName $computerName -argumentList $measure, $topProcess, $updateSpeed, $detailed {
            param ($measure, $topProcess, $updateSpeed, $detailed)
            ...
            if ($topProcess) {
                ...
                if ('NIC' -in $measure -or 'NIC' -in $detailed) {                
                    Get-WmiObject -Class Win32_NetworkAdapter -Property physicalAdapter, netEnabled, speed, name | where {$_.PhysicalAdapter -eq $true -and $_.NetEnabled -eq $true} | select @{n = 'name'; e = {$_.name -replace 'Intel\(R\)', 'Intel[R]'}}, speed | % { # prejmenovani Intel sitovek je hack, protze v counterech jsou s hranatymi zavorkami
            ...
            while (1) {
                ...
                if ($captureOutput -and [console]::KeyAvailable) {
                    $key = [system.console]::readkey($true)
                    if (($key.modifiers -band [consolemodifiers]"control") -and ($key.key -eq "C")) {
                        [console]::TreatControlCAsInput = $false
                        Write-Warning "Zachyceny vystup najdete v $path. Pro import pouzijte Import-Csv cestakcsv -Delimeter ';'"
                        break
                    }
                }
                ...
                if ($captureOutput) {
                    New-Object -TypeName PSObject -Property $result | Export-Csv $path -Append -NoTypeInformation -Delimiter ';' -Force -Encoding UTF8
                }
